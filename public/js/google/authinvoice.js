...
const thehr = sunset.nextElementSibling;
...
auth.onAuthStateChanged(user => {
	...
	if (user.displayName && user.email) {
		...
		pros.innerHTML = `Pro tip: Bank log files will be sent to <span>${user.email}</span> email inbox after any successful purchase`;
		...
		`;
		pros.innerHTML = `Pro tip: A link with bank log files will be sent to <span>${user.phoneNumber}</span> via sms after any successful purchase`;
		sunset.style.display = 'none';
		thehr.style.display = 'none';
	}  else if(user.phoneNumber && !user.displayName) {
		...
		`;
		pros.innerHTML = `Pro tip: Link an email when buying logs to get an <span>email invoice</span>`;
	} else if(user.isAnonymous && !user.displayName) {
